...
export async function GET() {
  ...
  try {
    const { rows } = await pool.query(`SELECT id,name,active,created_at FROM departments ORDER BY name`);
...
export async function POST(request: Request) {
  ...
  const pool = new Pool({ connectionString: url, max: 5 });
  try {
    const { rows } = await pool.query(`INSERT INTO departments (name) VALUES ($1) RETURNING id,name,active,created_at`, [name]);
