...
export type EOMFinalizationInput = { awardId: string; actorUserId?: string; bonusAmount: number; certificateNumber: string };
...
export async function finalizeEOMAward(input: EOMFinalizationInput) {
  ...
  try {
    ...
    await client.query(`INSERT INTO audit_events (action, actor_user_id, target_type, target_id, metadata) VALUES ('EOM_FINALIZED',$1,'AWARD',$2,$3::jsonb)`, [actor.id, input.awardId, JSON.stringify({ cycle: a.cycle, employeeId: a.employee_id, bonusAmount: input.bonusAmount })]);
