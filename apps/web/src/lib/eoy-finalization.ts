...
import { AWARD_APPROVERS } from './authorization';

export type EOYFinalizationInput = { awardId: string; actorUserId?: string; bonusAmount: number; certificateNumber: string };

export async function finalizeEOYAward(input: EOYFinalizationInput) {
  const actor = await requireUser(AWARD_APPROVERS);
  if (!input.awardId) throw new Error('Award is required');
  ...
  try {
    ...
    if (a.award_type !== 'EMPLOYEE_OF_YEAR') throw new Error('Award is not an Employee of the Year award');
    ...
    await client.query(`UPDATE awards SET status='APPROVED', approved_by=$2, finalized_at=now() WHERE id=$1`, [input.awardId, actor.id]);
    ...
    await client.query(`INSERT INTO audit_events (action, actor_user_id, target_type, target_id, metadata) VALUES ('EOY_FINALIZED',$1,'AWARD',$2,$3::jsonb)`, [actor.id, input.awardId, JSON.stringify({ cycle: a.cycle, employeeId: a.employee_id, bonusAmount: input.bonusAmount })]);
    await client.query('COMMIT');
    return { awardId: input.awardId, employeeId: a.employee_id, cycle: a.cycle, finalized: true, bonusAmount: input.bonusAmount, certificateNumber: input.certificateNumber.trim(), approvedBy: actor.id };
