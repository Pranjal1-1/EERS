...
export type EOYFinalizationInput = {
  awardId: string;
  actorUserId: string;
  bonusAmount: number;
...
export async function finalizeEOYAward(input: EOYFinalizationInput) {
  ...
  try {
    ...
    await client.query(`INSERT INTO audit_events (action, actor_user_id, target_type, target_id, metadata) VALUES ('EOY_FINALIZED',$1,'AWARD',$2,$3::jsonb)`, [input.actorUserId, input.awardId, JSON.stringify({ cycle: a.cycle, employeeId: a.employee_id, bonusAmount: input.bonusAmount })]);
